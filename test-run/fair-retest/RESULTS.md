# Fair re-test — confound-free EDIT comparison (v1.1)
...
- **Cases (6):** academic · marketing · notes · technical (from the generalization set) + two fiction
  cases written for this test: **fiction-a** (a narrator paragraph that explains what the scene
  shows) and **fiction-b** (voice-driven first-person reflection where no change is the expected answer).
...
- **Where S did worse: unauthorized loss of distinct information.**
  ...
  - *Fiction-a:* the manifest labelled Dana's realization paragraph the defect; **both judges
    ...
    and Dana's recognizing it — none of which the eggs or the drive establish. **All four arms lost
    the stated motive; S lost the most.** Ranking the least-damaging rewrite is not an improvement
    over the original (the untouched source was not a judged option — a design gap). A faithful
...
- **S vs K (Chen's *Human Scope*):** S better preserved the brief in both fiction cases. K's
  ...
  loss is the narrator's present confession. The other four cases are outside K's stated scope. This
...
- **Judges:** two independently run **OpenAI** models. That is not cross-provider corroboration,
  ...
  exact S-arm skill in [`S-arm-SKILL-snapshot.md`](S-arm-SKILL-snapshot.md) (sha256 `46c11ec0…a8d4`).

## Caveats (governing)
One sample per arm per case; 6 short cases; model judges (two families: GPT-6 Astra and GPT-5.6
Sol), not human readers. All generators are Claude, and the brief is edit-under-preservation, which
favors restraint by design. The v1.1 fixes below are derived from these same cases, so re-running
them on these cases would be overfitting. Validation needs **held-out cases**.
...
realization; keep light EDIT out of structural review; add a viewpoint-sensitive knowledge-boundary
