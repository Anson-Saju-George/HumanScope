...
**Protocol rule:** freeze the patched `SKILL.md` (record its sha256) **before** writing the new
source texts and before looking at any outputs.
...
| 2 | **Motive and realization — paired EDIT briefs.** A new close-third story states a motive that actions alone don't establish, plus a genuinely duplicated clause. | **2A** (ordinary light edit): keep the motive and realization, tighten the duplication. **2B** (explicit request to make the motive ambiguous): removing the explanation is allowed; keep the events. | The same deletion can't be right under both briefs. Count as one source cluster, not two independent cases. |
